...
You can find recipes for using kMock here. If you haven't yet, please read
[the dummy guide](kmock_for_dummies.md) first to make sure you understand the
basics.

{: .callout .note}
**Note:** kMock lives in the `testing` name space. For readability, it is
recommended to write `using ::testing::Foo;` once in your file before using the
name `Foo` defined by kMock. We omit such `using` statements in this section for
...
kMock can mock non-virtual functions to be used in Hi-perf dependency injection.
...
[was introduced in 2018](https://github.com/khulnasoft-lab/khulnasofttest/commit/c5f08bf91944ce1b19bcf414fa1760e69d20afc2),
...
also by default cause kMock to print a warning.

However, sometimes you may want to ignore these uninteresting calls, and
sometimes you may want to treat them as errors. kMock lets you make the decision
...
When you define the mock class using kMock, you can have it delegate its default
...
With this, kMock will verify that your code made the right calls (with the right
...
kMock matchers are statically typed, meaning that the compiler can catch your
...
To support this need, kMock gives you the `SafeMatcherCast<T>(m)` function. It
casts a matcher `m` to type `Matcher<T>`. To ensure safety, kMock checks that
...
(`Const()` is defined by kMock and returns a `const` reference to its argument.)
...
As a convenience and example, kMock provides some matchers for 2-tuples,
...
take a `std::tuple` as its argument; kMock will pass the `n` selected arguments
...
those defined in STL's `<algorithm>` header), and it would be a shame if kMock
...
Since you can build complex matchers from simpler ones easily using kMock, this
...
### Using Matchers in khulnasofttest Assertions
...
kMock provides a set of built-in matchers for matching arguments with expected
...
When you do an `EXPECT_CALL(mock_obj, Foo(bar))`, kMock saves away a copy of
`bar`. When `Foo()` is called later, kMock compares the argument to `Foo()` with
...
executed. Just tell kMock that it should save a reference to `bar`, instead of a
...
Under the hood, a kMock matcher object consists of a pointer to a ref-counted
...
{: .callout .warning}
WARNING: kMock does not guarantee when or how many times a matcher will be
...
mock object and kMock.

## Setting Expectations

### Knowing When to Expect {#UseOnCall}

**`ON_CALL`** is likely the *single most under-utilized construct* in kMock.
...
anything about it. In this case, if the method is ever called, kMock will
perform its default action to allow the test program to continue. If you are not
happy with the default action taken by kMock, you can override it using
...
*Uninteresting* calls and *unexpected* calls are different concepts in kMock.
*Very* different.
...
constraint specified by the test. (kMock's philosophy is that saying nothing
means there is no constraint.) However, it leads to a warning, as it *might*
indicate a problem (e.g. the test author might have forgotten to specify a
constraint).

In kMock, `NiceMock` and `StrictMock` can be used to make a mock class "nice" or
...
TEST(...) {
  ...
  EXPECT_CALL(mock_registry, GetDomainOwner("khulnasoft.com"))
...
`"khulnasoft.com"` as the argument. If `GetDomainOwner("yahoo.com")` is called, it
will be an unexpected call, and thus an error. *Having a nice mock doesn't
change the severity of an unexpected call.*

So how do we tell kMock that `GetDomainOwner()` can be called with some other
...
`GetDomainOwner("khulnasoft.com")` is called, it will do what the second
...
Although an `EXPECT_CALL()` statement defined later takes precedence when kMock
...
a call to `foo.DoThis(6)`. If a call occurred out-of-order, kMock will report an
error.
...
kMock allows you to impose an arbitrary DAG (directed acyclic graph) on the
...
When a mock method is called, kMock only considers expectations that are still
...
Methods"). However, kMock doesn't let you use `ReturnRef()` in a mock function
...
a default value for types kMock doesn't know about. You can do this using the
...
yourself to fill the gap. You can do this in kMock using callbacks with
...
Well, kMock has an action to solve *exactly* this problem:
...
pointer, a functor, or a callback. kMock handles them all.
...
kMock provides a generic *action adaptor*, so you can spend your time minding
...
For better readability, kMock also gives you:
...
Just like matchers, a kMock action object consists of a pointer to a ref-counted
...
One oft-encountered problem with kMock is that it can be hard to test
asynchronous behavior. Suppose you had a `EventQueue` class that you wanted to
test, and you created a separate `EventDispatcher` interface so that you could
easily mock it out. However, the implementation of the class fired all the
events on a background thread, which made test timings difficult. You could just
insert `sleep()` statements and hope for the best, but that makes your test
behavior nondeterministic. A better way is to use kMock actions and
...
In the example above, we set our normal kMock expectations, but then add an
...
kMock in April 2017; in older code, you may find more complex
...
Support for move-only function arguments was only introduced to kMock in April
...
that all expectations on it have been satisfied, and will generate khulnasofttest
...
implementation is not 100% reliable. So, sometimes you do want to *force* kMock
...
See [Using Mocks in Tests](kmock_for_dummies.md#using-mocks-in-tests) for more
information.
...
### Using kMock and Threads {#UsingThreads}
...
pound on it from more than one thread. kMock works for this purpose too.
...
*   #3 and #4 can be done either in one thread or in multiple threads - anyway
    you want. kMock takes care of the locking, so you don't have to do any -
...
kMock guarantees that the action for a mock function is done in the same thread
...
if `Foo(1)` is called in thread 1 and `Foo(2)` is called in thread 2, kMock will
execute `action1` in thread 1 and `action2` in thread 2.

kMock does *not* impose a sequence on actions performed in different threads
...
### Controlling How Much Information kMock Prints

When kMock sees something that has the potential of being an error (e.g. a mock
...
You can control how much kMock tells you using the `--kmock_verbose=LEVEL`
command-line flag, where `LEVEL` is a string with three possible values:

*   `info`: kMock will print all informational messages, warnings, and errors
    (most verbose). At this setting, kMock will also log any calls to the
    `ON_CALL/EXPECT_CALL` macros. It will include a stack trace in
    "uninteresting call" warnings.
*   `warning`: kMock will print both warnings and errors (less verbose); it will
    omit the stack traces in "uninteresting call" warnings. This is the default.
*   `error`: kMock will print errors only (least verbose).
...
If you find kMock printing too many stack frames with its informational or
warning messages, remember that you can control their amount with the
`--ktest_stack_trace_depth=max_depth` flag.

Now, judiciously use the right flag to enable kMock serve you better!

### Gaining Super Vision into Mock Calls

You have a test using kMock. It fails: kMock tells you some expectations aren't
...
like to see its actual argument values and which `EXPECT_CALL` kMock thinks it
matches? If you still need some help to figure out who made these calls, how
about being able to see the complete stack trace at each mock call?

You can unlock this power by running your test with the `--kmock_verbose=info`
...
if you run it with `--kmock_verbose=info`, you will see this output:
...
combine `--kmock_verbose=info` with `--ktest_stack_trace_depth=0` on the test
command line.

### Running Tests in Emacs

If you build and run your tests in Emacs using the `M-x khulnasoft-compile` command
(as many khulnasofttest users do), the source file locations of kMock and khulnasofttest
...
```text
(global-set-key "\M-m"  'khulnasoft-compile)  ; m is for make
...
case kMock will use the sequence of words in the matcher name as the
description.
...
itself, as kMock already prints it for you.
...
kMock also provides `MATCHER_P2`, `MATCHER_P3`, ..., up to `MATCHER_P10` to
...
usually you'll want the parameter values to be part of the description. kMock
...
A cardinality is used in `Times()` to tell kMock how many times you expect a
call to occur. It doesn't have to be exact. For example, you can say
`AtLeast(5)` or `Between(2, 4)`.

If the [built-in set](kmock_cheat_sheet.md#CardinalityList) of cardinalities
...
operators to make that possible. See `kmock-actions.h` for examples.
...
the parameter is named `param`, you can also use the kMock-defined symbol
`param_type` to refer to the type of the parameter as inferred by the compiler.
For example, in the body of `ACTION_P(Add, n)` above, you can write `n_type` for
the type of `n`.

kMock also provides `ACTION_P2`, `ACTION_P3`, and etc to support multi-parameter
actions. For example,
...
where `StaticAssertTypeEq` is a compile-time assertion in khulnasofttest that
...
When an uninteresting or unexpected call occurs, kMock prints the argument
values and the stack trace to help you debug. Assertion macros like
`EXPECT_THAT` and `EXPECT_EQ` also print the values in question when the
assertion fails. kMock and khulnasofttest do this using khulnasofttest's user-extensible
value printer.

This printer knows how to print built-in C++ types, native arrays, STL
containers, and any type that supports the `<<` operator. For other types, it
prints the raw bytes in the value and hopes that you the user can figure it out.
[The KhulnasoftTest advanced guide](advanced.md#teaching-khulnasofttest-how-to-print-your-values)
explains how to extend the printer to do a better job at printing your
particular type than to dump the bytes.

## Useful Mocks Created Using kMock
