...
This page lists the facilities provided by KhulnasoftTest for writing test programs.
To use them, add `#include <ktest/ktest.h>`.

## Macros

KhulnasoftTest defines the following macros for writing tests.
...
The argument *`param_generator`* is one of the following KhulnasoftTest-provided
...
KhulnasoftTest provides [`PrintToStringParamName`](#PrintToStringParamName), or a
...
test in the test suite `KhulnasoftTestVerification`.
`KTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST` suppresses this failure for the
given test suite.

## Classes and types

KhulnasoftTest defines the following classes and types to help with writing tests.
...
Performs shared setup for all tests in the test suite. KhulnasoftTest calls
...
Performs shared teardown for all tests in the test suite. KhulnasoftTest calls
...
already used by KhulnasoftTest (`name`, `file`, `line`, `status`, `time`,
...
Override this to perform test fixture setup. KhulnasoftTest calls `SetUp()` before
...
Override this to perform test fixture teardown. KhulnasoftTest calls `TearDown()`
...
KhulnasoftTest allows the user to filter the tests by their full names. Only the
...
KhulnasoftTest. See [`TestEventListeners`](#TestEventListeners).
...
iteration if `KTEST_FLAG(repeat)` is set. `iteration` is the iteration index,
starting from 0.
...
Lets users add listeners to track events in KhulnasoftTest.
...
Appends an event listener to the end of the list. KhulnasoftTest assumes ownership
...
by the `--ktest_output=xml` flag. Can be removed from the listeners list by
...
KhulnasoftTest defines the following functions to help with writing and running
tests.

### InitKhulnasoftTest {#InitKhulnasoftTest}

`void testing::InitKhulnasoftTest(int* argc, char** argv)` \
`void testing::InitKhulnasoftTest(int* argc, wchar_t** argv)` \
`void testing::InitKhulnasoftTest()`

Initializes KhulnasoftTest. This must be called before calling
[`RUN_ALL_TESTS()`](#RUN_ALL_TESTS). In particular, it parses the command line
for the flags that KhulnasoftTest recognizes. Whenever a KhulnasoftTest flag is seen, it
is removed from `argv`, and `*argc` is decremented.

No value is returned. Instead, the KhulnasoftTest flag variables are updated.

The `InitKhulnasoftTest(int* argc, wchar_t** argv)` overload can be used in Windows
programs compiled in `UNICODE` mode.

The argument-less `InitKhulnasoftTest()` overload can be used on Arduino/embedded
...
Prints any value `x` using KhulnasoftTest's value printer.
