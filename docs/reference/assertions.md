...
This page lists the assertion macros provided by KhulnasoftTest for verifying code
behavior. To use them, add `#include <ktest/ktest.h>`.
...
user-visible output. However, we may add `SUCCEED` messages to KhulnasoftTest output
in the future.
...
KhulnasoftTest provides a built-in library of matchers—see the
[Matchers Reference](matchers.md). It is also possible to write your own
matchers—see [Writing New Matchers Quickly](../kmock_cook_book.md#NewMatchers).
...
argument when the assertion fails. Otherwise, KhulnasoftTest will attempt to print
them in the best way it can—see
[Teaching KhulnasoftTest How to Print Your Values](../advanced.md#teaching-khulnasofttest-how-to-print-your-values).
...
KhulnasoftTest also provides assertions that use a default error bound based on
...
`::testing::KTEST_FLAG(death_test_style)`, which is initialized from the
command-line flag `--ktest_death_test_style`.
...
status and returns a `bool`. KhulnasoftTest provides two predicates to handle common
cases:
