...
The above matchers use ULP-based comparison (the same as used in khulnasofttest).
...
| `SafeMatcherCast<T>(m)` | [safely casts](../kmock_cook_book.md#SafeMatcherCast) matcher `m` to type `Matcher<T>`. |
