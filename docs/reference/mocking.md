...
This page lists the facilities provided by KhulnasoftTest for creating and working
with mock objects. To use them, add `#include <kmock/kmock.h>`.

## Macros {#macros}

KhulnasoftTest defines the following macros for working with mocks.
...
Creates an [expectation](../kmock_for_dummies.md#setting-expectations) that the
...
[matchers](../kmock_for_dummies.md#matchers-what-arguments-do-we-expect) that
...
KhulnasoftTest passes all of the arguments as one tuple into the matcher. The
...
KhulnasoftTest provides some built-in matchers for 2-tuples, including the `Lt()`
...
If the `Times` clause is omitted, KhulnasoftTest infers the cardinality as follows:
...
[action](../kmock_for_dummies.md#actions-what-should-it-do) that the function
...
KhulnasoftTest defines the following classes for working with mocks.
...
[uninteresting calls](../kmock_cook_book.md#uninteresting-vs-unexpected). The
