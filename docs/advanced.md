# Advanced KhulnasoftTest Topics

## Introduction

Now that you have read the [KhulnasoftTest Primer](primer.md) and learned how to
write tests using KhulnasoftTest, it's time to learn some new tricks. This document
...
Even though KhulnasoftTest has a rich set of assertions, they can never be complete,
...
KhulnasoftTest gives you three different options to solve this problem:
...
(one third of Boolean assertions in the Khulnasoft code base are negative ones), and
...
### Asserting Using kMock Matchers
...
(Please read the [previous](#asserting-using-kmock-matchers) section first if
you haven't.)

You can use the kMock [string matchers](reference/matchers.md#string-matchers)
...
void-returning functions. This is a consequence of Khulnasoft's not using
...
execution at runtime with the `KTEST_SKIP()` macro. This is useful when you need
to check for preconditions of the system under test during runtime and skip
tests in a meaningful way.

`KTEST_SKIP()` can be used in individual test cases or in the `SetUp()` methods
...
As with assertion macros, you can stream a custom message into `KTEST_SKIP()`.

## Teaching KhulnasoftTest How to Print Your Values

When a test assertion such as `EXPECT_EQ` fails, KhulnasoftTest prints the argument
...
class Point {  // We want KhulnasoftTest to be able to print instances of this.
  ...
...
used by KhulnasoftTest. This allows you to customize how the value appears in
KhulnasoftTest's output without affecting code that relies on the behavior of
`AbslStringify()`.

If you have an existing `<<` operator and would like to define an
`AbslStringify()`, the latter will be used for KhulnasoftTest printing.

If you want to print a value `x` using KhulnasoftTest's value printer yourself, just
...
KhulnasoftTest provides assertion macros to support death tests. See
...
will **not** cause the death test to fail, as KhulnasoftTest assertions don't abort
the process.
...
When built with Bazel and using Abseil, KhulnasoftTest uses the
[RE2](https://github.com/khulnasoft-lab/re2/wiki/Syntax) syntax. Otherwise, for POSIX
systems (Linux, Cygwin, Mac), KhulnasoftTest uses the
...
On Windows, KhulnasoftTest uses its own simple regular expression implementation. It
...
To help you determine which capability is available on your system, KhulnasoftTest
defines macros to govern which regular expression it is using. The macros are:
`KTEST_USES_SIMPLE_RE=1` or `KTEST_USES_POSIX_RE=1`. If you want your death
...
KhulnasoftTest has three features intended to raise awareness of threading issues.
...
exception, the death test is considered to have failed. Some KhulnasoftTest macros
...
[a custom KMock matcher](kmock_cook_book.md#NewMatchers) instead. This lets you
...
To alleviate this, KhulnasoftTest provides three different solutions. You could use
...
KhulnasoftTest offers the following macros:
...
>     ones already used by KhulnasoftTest (`name`, `status`, `time`, `classname`,
...
KhulnasoftTest creates a new test fixture object for each test in order to make
tests independent and easier to debug. However, sometimes tests use resources
that are expensive to set up, making the one-copy-per-test model prohibitively
expensive.

If the tests don't change the resource, there's no harm in their sharing a
single resource copy. So, in addition to per-test set-up/tear-down, KhulnasoftTest
...
That's it! KhulnasoftTest automatically calls `SetUpTestSuite()` before running the
...
Then, you register an instance of your environment class with KhulnasoftTest by
...
reported fatal failures and `KTEST_SKIP()` was not called. `RUN_ALL_TESTS()`
...
Note that KhulnasoftTest takes ownership of the registered environment objects.
Therefore **do not delete them** by yourself.

You should call `AddGlobalTestEnvironment()` before `RUN_ALL_TESTS()` is called,
probably in `main()`. If you use `ktest_main`, you need to call this before
...
test suite with any set of parameters you want. KhulnasoftTest defines a number of
...
The parameter generator expression is not evaluated until KhulnasoftTest is
initialized (via `InitKhulnasoftTest()`). Any prior initialization done in the
...
You can use these names in [`--ktest_filter`](#running-a-subset-of-the-tests).
...
`KhulnasoftTestVerification`. If you have a test suite where that omission is not an
...
[sample7_unittest.cc]: https://github.com/khulnasoft-lab/khulnasofttest/blob/main/khulnasofttest/samples/sample7_unittest.cc "Parameterized Test example"
[sample8_unittest.cc]: https://github.com/khulnasoft-lab/khulnasofttest/blob/main/khulnasofttest/samples/sample8_unittest.cc "Parameterized Test example with multiple parameters"
...
[sample6_unittest.cc]: https://github.com/khulnasoft-lab/khulnasofttest/blob/main/khulnasofttest/samples/sample6_unittest.cc "Typed Test example"
...
If you are building a testing utility on top of KhulnasoftTest, you'll want to test
your utility. What framework would you use to test it? KhulnasoftTest, of course.

The challenge is to verify that your testing utility reports failures correctly.
In frameworks that report a failure by throwing an exception, you could catch
the exception and assert on it. But KhulnasoftTest doesn't use exceptions, so how do
we test that a piece of code generates an expected failure?

`"ktest/ktest-spi.h"` contains some constructs to do this.
...
## Extending KhulnasoftTest by Handling Test Events

KhulnasoftTest provides an **event listener API** to let you receive notifications
...
KhulnasoftTest event listener list (represented by class
...
int main(int argc, char** argv) {
  ...
  // Adds a listener to the end.  KhulnasoftTest takes the ownership.
...
[sample9_unittest.cc]: https://github.com/khulnasoft-lab/khulnasofttest/blob/main/khulnasofttest/samples/sample9_unittest.cc "Event listener example"
...
[sample10_unittest.cc]: https://github.com/khulnasoft-lab/khulnasofttest/blob/main/khulnasofttest/samples/sample10_unittest.cc "Failure-raising listener example"

## Running Test Programs: Advanced Options

KhulnasoftTest test programs are ordinary executables. Once built, you can run them
directly and affect their behavior via the following environment variables
and/or command line flags. For the flags to work, your programs must call
`::testing::InitKhulnasoftTest()` before calling `RUN_ALL_TESTS()`.
...
`--ktest_list_tests` overrides all other flags and lists tests in the following
format:
...
By default, a KhulnasoftTest program runs all tests the user has defined. Sometimes,
you want to run only a subset of the tests (e.g. for debugging or quickly
verifying a change). If you set the `KTEST_FILTER` environment variable or the
`--ktest_filter` flag to a filter string, KhulnasoftTest will only run the tests
...
*   `./foo_test --ktest_filter=*` Also runs everything, due to the single
    match-everything `*` value.
*   `./foo_test --ktest_filter=FooTest.*` Runs everything in test suite
    `FooTest` .
*   `./foo_test --ktest_filter=*Null*:*Constructor*` Runs any test whose full
    name contains either `"Null"` or `"Constructor"` .
*   `./foo_test --ktest_filter=-*DeathTest.*` Runs all non-death tests.
*   `./foo_test --ktest_filter=FooTest.*-FooTest.Bar` Runs everything in test
    suite `FooTest` except `FooTest.Bar`.
*   `./foo_test --ktest_filter=FooTest.*:BarTest.*-FooTest.Bar:BarTest.Foo` Runs
    everything in test suite `FooTest` except `FooTest.Bar` and everything in
    test suite `BarTest` except `BarTest.Foo`.

#### Stop test execution upon first failure

By default, a KhulnasoftTest program runs all tests the user has defined. In some
cases (e.g. iterative test development & execution) it may be desirable stop
test execution upon first failure (trading improved latency for completeness).
If `KTEST_FAIL_FAST` environment variable or `--ktest_fail_fast` flag is set,
...
For example, the following tests won't be run by KhulnasoftTest, even though they
...
to fix the disabled tests at a later date. As a reminder, KhulnasoftTest will print
...
the `--ktest_also_run_disabled_tests` flag or set the
`KTEST_ALSO_RUN_DISABLED_TESTS` environment variable to a value other than `0`.
You can combine this with the `--ktest_filter` flag to further select which
...
The `--ktest_repeat` flag allows you to repeat all (or selected) test methods in
...
`--ktest_recreate_environments_when_repeating=false`{.nowrap}.

You can also specify the repeat count by setting the `KTEST_REPEAT` environment
variable.

### Shuffling the Tests

You can specify the `--ktest_shuffle` flag (or set the `KTEST_SHUFFLE`
environment variable to `1`) to run the tests in a program in a random order.
This helps to reveal bad dependencies between tests.

By default, KhulnasoftTest uses a random seed calculated from the current time.
Therefore you'll get a different order every time. The console output includes
the random seed value, such that you can reproduce an order-related test failure
later. To specify the random seed explicitly, use the `--ktest_random_seed=SEED`
flag (or set the `KTEST_RANDOM_SEED` environment variable), where `SEED` is an
integer in the range [0, 99999]. The seed value 0 is special: it tells
KhulnasoftTest to do the default behavior of calculating the seed from the current
time.

If you combine this with `--ktest_repeat=N`, KhulnasoftTest will pick a different
...
KhulnasoftTest is compatible with test sharding. To take advantage of this feature,
your test runner (not part of KhulnasoftTest) needs to do the following:

1.  Allocate a number of machines (shards) to run the tests.
1.  On each shard, set the `KTEST_TOTAL_SHARDS` environment variable to the total
    number of shards. It must be the same for all shards.
1.  On each shard, set the `KTEST_SHARD_INDEX` environment variable to the index
    of the shard. Different shards must be assigned different indices, which
    must be in the range `[0, KTEST_TOTAL_SHARDS - 1]`.
1.  Run the same test program on all shards. When KhulnasoftTest sees the above two
...
Your project may have tests that were written without KhulnasoftTest and thus don't
understand this protocol. In order for your test runner to figure out which test
supports sharding, it can set the environment variable `KTEST_SHARD_STATUS_FILE`
...
parallel, you would set `KTEST_TOTAL_SHARDS` to 3 on all machines, and set
`KTEST_SHARD_INDEX` to 0, 1, and 2 on the machines respectively. Then you would
run the same `foo_test` on each machine.

KhulnasoftTest reserves the right to change how the work is distributed across the
...
KhulnasoftTest can use colors in its terminal output to make it easier to spot the
...
You can set the `KTEST_COLOR` environment variable or the `--ktest_color`
command line flag to `yes`, `no`, or `auto` (the default) to enable colors,
disable colors, or let KhulnasoftTest decide. When the value is `auto`, KhulnasoftTest
will use colors if and only if the output goes to a terminal and (on non-Windows
platforms) the `TERM` environment variable is set to `xterm` or `xterm-color`.

#### Suppressing test passes

By default, KhulnasoftTest prints 1 line of output for each test, indicating if it
passed or failed. To show only test failures, run the test program with
`--ktest_brief=1`, or set the KTEST_BRIEF environment variable to `1`.

#### Suppressing the Elapsed Time

By default, KhulnasoftTest prints the time it takes to run each test. To disable
that, run the test program with the `--ktest_print_time=0` command line flag, or
set the KTEST_PRINT_TIME environment variable to `0`.

#### Suppressing UTF-8 Text Output

In case of assertion failures, KhulnasoftTest prints expected and actual values of
type `string` both as hex-encoded strings as well as in readable UTF-8 text if
they contain valid non-ASCII UTF-8 characters. If you want to suppress the UTF-8
text because, for example, you don't have an UTF-8 compatible output medium, run
the test program with `--ktest_print_utf8=0` or set the `KTEST_PRINT_UTF8`
environment variable to `0`.

#### Generating an XML Report

KhulnasoftTest can emit a detailed XML report to a file in addition to its normal
textual output. The report contains the duration of each test, and thus can help
you identify slow tests.

To generate the XML report, set the `KTEST_OUTPUT` environment variable or the
`--ktest_output` flag to the string `"xml:path_to_output_file"`, which will
create the file at the given location. You can also just use the string `"xml"`,
in which case the output can be found in the `test_detail.xml` file in the
current directory.

If you specify a directory (for example, `"xml:output/directory/"` on Linux or
`"xml:output\directory\"` on Windows), KhulnasoftTest will create the XML file in
that directory, named after the test executable (e.g. `foo_test.xml` for test
program `foo_test` or `foo_test.exe`). If the file already exists (perhaps left
over from a previous run), KhulnasoftTest will pick a different name (e.g.
`foo_test_1.xml`) to avoid overwriting it.

The report is based on the `junitreport` Ant task. Since that format was
originally intended for Java, a little interpretation is required to make it
apply to KhulnasoftTest tests, as shown here:
...
*   `<testsuite>` elements correspond to KhulnasoftTest test suites.
*   `<testcase>` elements correspond to KhulnasoftTest test functions.
...
*   The `tests` attribute of a `<testsuites>` or `<testsuite>` element tells how
    many test functions the KhulnasoftTest program or test suite contains, while the
...
*   Each `<failure>` element corresponds to a single failed KhulnasoftTest
    assertion.

#### Generating a JSON Report

KhulnasoftTest can also emit a JSON report as an alternative format to XML. To
generate the JSON report, set the `KTEST_OUTPUT` environment variable or the
`--ktest_output` flag to the string `"json:path_to_output_file"`, which will
...
[JSON encoding](https://developers.khulnasoft.com/protocol-buffers/docs/proto3#json):

```proto
syntax = "proto3";

package khulnasofttest;

import "khulnasoft-lab/protobuf/timestamp.proto";
import "khulnasoft-lab/protobuf/duration.proto";
...
message UnitTest {
  ...
  int32 disabled = 3;
  int32 errors = 4;
  khulnasoft.protobuf.Timestamp timestamp = 5;
  khulnasoft.protobuf.Duration time = 6;
  string name = 7;
...
message TestInfo {
  ...
  Status status = 2;
  khulnasoft.protobuf.Duration time = 3;
  string classname = 4;
  message Failure {
    string failures = 1;
    string type = 2;
...
Khulnasoft Test implements the _premature-exit-file_ protocol for test runners to
catch any kind of unexpected exits of test programs. Upon start, Khulnasoft Test
...
mode. KhulnasoftTest's *break-on-failure* mode supports this behavior.

To enable it, set the `KTEST_BREAK_ON_FAILURE` environment variable to a value
other than `0`. Alternatively, you can use the `--ktest_break_on_failure`
command line flag.

#### Disabling Catching Test-Thrown Exceptions

KhulnasoftTest can be used either with or without exceptions enabled. If a test
throws a C++ exception or (on Windows) a structured exception (SEH), by default
KhulnasoftTest catches it, reports it as a test failure, and continues with the next
...
exception is thrown. To achieve that, set the `KTEST_CATCH_EXCEPTIONS`
environment variable to `0`, or use the `--ktest_catch_exceptions=0` flag when
running the tests.
...
[Address Sanitizer](https://github.com/khulnasoft-lab/sanitizers/wiki/AddressSanitizer),
and
[Thread Sanitizer](https://github.com/khulnasoft-lab/sanitizers/wiki/ThreadSanitizerCppManual)
...
test triggers a sanitizer error, KhulnasoftTest will report that it failed.
